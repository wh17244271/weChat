...
public class StringUtils {
     ...
     *
     * @param value   检验字段
     * @param message 需要提醒的字段
     */
    public static void isEntryException(String value, String message) {
        if (isEmpty(value)) {
            throw new VenusResponseException(message + "不能为空");
        }
    }
    ...
    /**
     * 判断数组对象是否为空，并抛出不能为空异常
     *
     * @param value
     * @param message
     * @param <T>
     */
    public static <T> void assertIsEmpty(T[] value, String message) {
        if (isEmpty(value)) {
            throw new VenusResponseException(message + "转换失败");
        }
    }
